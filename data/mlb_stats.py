...
def get_games_for_date(date_str):
    # 1. Try Official MLB API (Standard schedule endpoint)
    mlb_url = f"https://statsapi.mlb.com/api/v1/schedule?sportId=1&date={date_str}"
    try:
...
def update_html():
    ...
    # Safety Check: If we found no games after checking 7 days, do nothing.
    if not table_rows:
        print("No games found this week. Keeping existing index.html.")
        return
    ...
    <table border="1"><tr><th>Date</th><th>Matchup</th></tr>{''.join(table_rows)}</table>
    <p>Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}</p></body></html>"""
    
    with open(output_path, "w") as f:
        f.write(html)
    print("Successfully updated index.html with new schedule.")
