...
def get_game_details(game_pk):
    # Fetch boxscore data for the specific game ID
...
for index, row in df_games.iterrows():
    ...
    # Create an expander for each game to show stats/rosters
    with st.expander(f"{away_team} @ {home_team}"):
        if st.button(f"Load Details for Game {game_pk}", key=f"btn_{game_pk}"):
            with st.spinner("Fetching stats..."):
                ...
                # Logic to parse rosters from the boxscore
                away_players = details['teams']['away']['players']
                # Create a list of player names
                roster = [player_data['person']['fullName'] for player_id, player_data in away_players.items()]
                
                st.write("### Away Roster")
                st.write(roster)
                
                # Logic to parse stats (simplified example)
