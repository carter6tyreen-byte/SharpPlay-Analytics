...
def load_data():
    try:
        ...
        # Access the 'games' list nested inside the first item of 'dates'
        # This navigates: data -> dates (list) -> [0] -> games (list)
        games_list = data.get("dates", [{}])[0].get("games", [])
        
        if not games_list:
            return None
            
        # Flatten the nested JSON structure into a clean table
...
if df is not None and not df.empty:
    ...
    # Selecting relevant columns for a clean view
    # Note: These column names are generated by json_normalize from your nested structure
    display_cols = [
